function dragElement(elmnt) {
    ...
    function getWordRangeAtPoint(x, y) {
        ...
        if (range && range.startContainer.nodeType === Node.TEXT_NODE) {
            ...
            while (start > 0 && range.startContainer.textContent[start - 1].match(/\S/)) {
                start--;
            }
            while (end < range.startContainer.textContent.length && range.startContainer.textContent[end].match(/\S/)) {
                end++;
            }
